...
def report_to_json()->None:
    ...
    os.makedirs(REPORT_DIR,exist_ok=True) #ensure that reporting directory exists
    for file_name in os.listdir(path=DIR):
        ...
        with open(f"{DIR}/{file_name}",encoding="UTF-8") as file:
            ...
            if result:
                with open(f"{REPORT_DIR}/{file_name_no_suff}{datetime.now()}.json",mode="w") as w_file:
        ...
        with open(f"{DIR}/{file_name}",encoding="UTF-8") as file:
            ...
            with connection.cursor() as cursor:
                ...
                if result:
                    ...
                    with open(f"{REPORT_DIR}/{file_name_no_suff}{datetime.now()}.xml",mode="w") as w_file:
                        from xml.dom.minidom import parseString
                        raw_xml=dicttoxml(result[0],custom_root=file_name_no_suff,
                                           item_func=lambda _:"row", attr_type=False,return_bytes=False)
                        to_write=parseString(raw_xml).toprettyxml(indent="   ")
                        if isinstance(to_write,str):#dummy check for pyliance
